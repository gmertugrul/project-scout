...
async function BuyInfoCard({ player }: { player: Player }) {
  return (
    <div className="card grid grid-cols-2 items-center">
      <h2 className="font-semibold">Min Price</h2>
      ...
      <h2 className="font-semibold">Shares Listed</h2>
      <p className="text-gray-500 text-sm text-right">700</p>

      <hr className="col-span-2 my-4 -mx-card" />

      <h2 className="font-semibold">Buying Power</h2>
      <p className="text-gray-500 text-sm text-right">
        <strong>700</strong> shares
      </p>
    </div>
  );
}
    ...
    <div className="flex flex-col gap-2">
      {list.map((x) => (
        <div key={x} className="card flex items-center gap-3">
          ...
          <BuyModal
            player={player}
            trigger={<button className="btn-green">Buy Now</button>}
          />
        </div>
      ))}
    </div>
